...
Failures are deliberately excluded so the nightly run never retries known-bad documents forever; only an explicit `retryFailures` run reconsiders them. Scanning is once-per-document: a scanned document (with `document_citations` rows, or a `completed` row) is never reselected automatically, including after a parser-version bump — reprocessing happens only through an explicit forced `reprocess` run, which drops every gate and re-scans all streamable in-scope documents, replacing their citations. An operator may also pass `autoContinue` to chain batches and drain the whole backlog in one run; the nightly run stays a single bounded page.
